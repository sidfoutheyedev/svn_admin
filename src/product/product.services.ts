    ...
    { $unwind: { path: "$swipe_stats", preserveNullAndEmptyArrays: true } },
    {
      $lookup: {
        ...
        pipeline: [
          ...
          {
            $addFields: {
              sku: "$sku_docs.sku_code",
              sold_stock: {
                $size: {
                  $filter: {
                    input: "$sku_docs",
                    as: "s",
                    cond: { $eq: ["$$s.is_sold", true] },
                  },
                },
              },
            },
        ...
        product_varient: {
          $map: {
            ...
            in: {
              ...
              discount_price: "$$variant.discount_price",
              stock_on_hand: "$$variant.stock_on_hand",
              sold_stock: { $ifNull: ["$$variant.sold_stock", 0] },
              stock_left: { $ifNull: ["$$variant.stock_on_hand", 0] }, // remaining
              total_stock: {
                $add: [
                  { $ifNull: ["$$variant.stock_on_hand", 0] },
                  { $ifNull: ["$$variant.sold_stock", 0] },
                ],
              },
              product_images: "$$variant.product_images",
              is_default: "$$variant.is_default",
